...
5. VLM 兜底第一轮应优先使用云端模型，先验证收益，再承担本地部署复杂度。
...
VLM 兜底第一轮先用云端模型，不先部署本地 VLM。理由是先验证“VLM 是否能在疑难图纸上产生增益”，再决定是否为私有化承担本地模型部署成本。
...
4. 选 4-8 张疑难样本跑云端 VLM 兜底小实验，要求输出固定 JSON。
5. 若云端 VLM 证明有增益，再按配置压力测试本地模型：SmolVLM/Florence -> InternVL2.5-1B -> Qwen2.5-VL-3B -> MiniCPM-V-2.6。
6. 比较：

# 文档索引

本目录存放公开、可提交的项目文档。私有图纸、渲染图片、标注结果和训练数据不放在这里。

## 分类

- `research/`：调研笔记、横向对标、技术路线依据。
- `plans/`：阶段计划、实验计划、实现前规划。
- `workflows/`：人工操作流程、标注工具使用说明。

## 推荐阅读顺序

1. [YOLO/OBB 训练规划依据](research/2026-06-26-yolo-obb-training-basis.md)
2. [YOLO/OBB 调试方案取经笔记](research/2026-06-25-yolo-obb-debugging-research.md)
3. [YOLO/OBB 标题栏检测小实验计划](plans/yolo-obb-title-block-experiment-plan.md)
4. [ISAT OBB 标注流程](workflows/isat-obb-annotation-workflow.md)

## Research

- [工程图纸方向识别调研](research/2026-06-23-engineering-drawing-orientation-research.md)
- [OCR/VLM 工作流调研](research/2026-06-25-ocr-vlm-workflow-research.md)
- [本地标题栏检测与 VLM 调研](research/2026-06-25-local-title-block-detector-and-vlm-research.md)
- [OBB 标注工具选择调研](research/2026-06-25-obb-annotation-tool-selection.md)
- [YOLO/OBB 调试方案调研](research/2026-06-25-yolo-obb-debugging-research.md)
- [ISAT 标注工具调研](research/2026-06-26-isat-annotation-tool-research.md)
- [YOLO/OBB 训练规划依据](research/2026-06-26-yolo-obb-training-basis.md)

## Plans

- [旋转方向识别计划](plans/rotation-detection-plan.md)
- [置信度提升计划](plans/confidence-improvement-plan.md)
- [三方比对计划](plans/three-way-rotation-comparison-plan.md)
- [OpenCV 阶段二修复计划](plans/opencv-stage2-error-fix-plan.md)
- [Ground truth 与评估计划](plans/ground-truth-evaluation-plan.md)
- [人工复核包计划](plans/manual-review-pack-plan.md)
- [顺时针 90 度增强计划](plans/augmented-90-sample-plan.md)
- [联合评估计划](plans/combined-evaluation-plan.md)
- [sample_042 低置信优化计划](plans/low-confidence-042-plan.md)
- [OCR/VLM 兜底流程计划](plans/ocr-vlm-fallback-workflow-plan.md)
- [YOLO/OBB 标题栏检测计划](plans/yolo-obb-title-block-experiment-plan.md)
- [YOLO/OBB 调试调研计划](plans/yolo-obb-debugging-research-plan.md)
- [YOLO/OBB 标签工具计划](plans/yolo-obb-label-tools-plan.md)
- [YOLO/OBB 冒烟训练准备计划](plans/yolo-obb-smoke-training-plan.md)
- [标注工具选择计划](plans/annotation-tool-selection-plan.md)
- [ISAT 调研计划](plans/isat-annotation-tool-research-plan.md)
- [项目结构整理计划](plans/project-structure-cleanup-plan.md)

## Workflows

- [ISAT OBB 标注流程](workflows/isat-obb-annotation-workflow.md)
- [Labelme OBB 标注流程](workflows/labelme-obb-annotation-workflow.md)
